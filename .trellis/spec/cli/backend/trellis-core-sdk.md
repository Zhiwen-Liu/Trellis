...
> Package boundary and coding rules for `@zhiwenliu/trellis-core` and the CLI.
...
|---|---|
| `@zhiwenliu/trellis-core` | Reusable domain logic, storage primitives, reducers, task APIs, channel APIs, and typed contracts. |
| `@zhiwenliu/trellis` | CLI argument parsing, terminal rendering, command wiring, process exit behavior, template installation, migrations, and release scripts. |
...
```ts
import { createChannelStore } from "@zhiwenliu/trellis-core/channel";
```
...
```bash
pnpm --filter @zhiwenliu/trellis-core build
pnpm --filter @zhiwenliu/trellis typecheck
```
