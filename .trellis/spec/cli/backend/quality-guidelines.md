...
| Kind | Example | Action |
|------|---------|--------|
| **Schema / creator** | `task_store.cmd_create`, `core/task:emptyTaskRecord` (re-exported by `utils/task-json.ts:emptyTaskJson` for legacy CLI call sites) | Drop field from output |
...
`core/task` as `TrellisTaskRecord` +
