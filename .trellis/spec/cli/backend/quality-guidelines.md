...
| Kind | Example | Action |
|------|---------|--------|
| **Schema / creator** | `task_store.cmd_create`, `@zhiwenliu/trellis-core/task:emptyTaskRecord` (re-exported by `utils/task-json.ts:emptyTaskJson` for legacy CLI call sites) | Drop field from output |
...
`@zhiwenliu/trellis-core/task` as `TrellisTaskRecord` +
