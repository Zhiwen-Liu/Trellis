...
`core/channel` owns reusable channel domain behavior:
...
metadataFromCreateEvent(ev?): ChannelMetadata
  ...
  // core/channel and do not call from CLI renderers.
...
**Worker lifecycle / inbox / delivery contracts** (owned by `core/channel`):
...
- Trigger: `core/channel` mutation APIs need replay safety for
