...
`@zhiwenliu/trellis-core/channel` owns reusable channel domain behavior:
...
metadataFromCreateEvent(ev?): ChannelMetadata
  ...
  // @zhiwenliu/trellis-core/channel and do not call from CLI renderers.
...
**Worker lifecycle / inbox / delivery contracts** (owned by `@zhiwenliu/trellis-core`):
...
- Trigger: `@zhiwenliu/trellis-core` mutation APIs need replay safety for
