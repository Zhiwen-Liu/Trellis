...
- The `trellis-kerminal` package owns both sides: reusable channel/thread state, event append, seq allocation, context/title projection, reducers, and task helpers live in its core module, while the CLI layer owns flags, terminal rendering, prompts, worker lifecycle, and process exits.
