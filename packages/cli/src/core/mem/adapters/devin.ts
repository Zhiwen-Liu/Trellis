...
function bundleOf(store: DevinSessionStore, sessionId: string): SessionBundle {
...
function readSessionBundle(
  ...
  return (
    loadStore(dbPath, warnings, sessionId).bundles.get(sessionId) ?? {
      nodes: [],
    }
  );
}
