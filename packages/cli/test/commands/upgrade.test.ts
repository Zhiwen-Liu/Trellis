...
describe("upgrade command", () => {
  ...
  it("builds POSIX npm global install command without shell", () => {
    expect(
      ...
      command: "npm",
      args: ["install", "-g", "trellis-kerminal@beta"],
      spawnOptions: { stdio: "inherit", shell: false },
      displayCommand: "npm install -g trellis-kerminal@beta",
      target: "trellis-kerminal@beta",
      tag: "beta",
  ...
  it("builds Windows command through cmd.exe", () => {
    expect(
      ...
      command: "cmd.exe",
      args: ["/d", "/s", "/c", "npm install -g trellis-kerminal@beta"],
      spawnOptions: { stdio: "inherit", shell: false },
      displayCommand: "npm install -g trellis-kerminal@beta",
      target: "trellis-kerminal@beta",
      tag: "beta",
  ...
  it("dry-run does not execute npm", async () => {
    ...
    expect(log).toHaveBeenCalledWith(
      expect.stringContaining("Run: npm install -g trellis-kerminal@latest"),
    );
  ...
  it("executes npm install for real upgrades", async () => {
    ...
    expect(runner).toHaveBeenCalledWith(
      "npm",
      ["install", "-g", "trellis-kerminal@latest"],
  ...
  it("fails when npm exits non-zero", async () => {
    ...
    await expect(upgrade({ tag: "latest" }, runner)).rejects.toThrow(
      /npm install failed with exit code 1\.[\s\S]*Troubleshooting:[\s\S]*Manual command: npm install -g trellis-kerminal@latest[\s\S]*npm config get prefix[\s\S]*which trellis/,
    );
