...
description: |
  ...
  then implements features. No git commit allowed. On Kerminal the main
  session loads this skill (`.kerminal/skills/trellis-implement/SKILL.md`)
  and spawns a generic sub-agent whose prompt is this file's content; the
  first prompt line must be Active task: <path>.
tools: shell_command, read_file, write_file, edit_file, view_image, web_search, update_plan
---
...
1. Load the `trellis-implement` skill (`.kerminal/skills/trellis-implement/SKILL.md`).
2. Spawn a generic sub-agent whose initial message is the skill content, prefixed with:
   ...
   - A statement that the spawned agent is already `trellis-implement` and must implement directly without spawning another `trellis-implement` / `trellis-check`

Kerminal auto-injects the project `AGENTS.md` into spawned sub-agents, so project-level conventions arrive automatically. Task context (active task path, specs, artifacts) is pulled by the prelude below.
