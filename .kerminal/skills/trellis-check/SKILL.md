...
description: |
  ...
  and self-fixes issues. On Kerminal the main session loads this skill
  (`.kerminal/skills/trellis-check/SKILL.md`) and spawns a generic sub-agent
  whose prompt is this file's content; the first prompt line must be
  Active task: <path>.
...
1. Load the `trellis-check` skill (`.kerminal/skills/trellis-check/SKILL.md`).
2. Spawn a generic sub-agent whose initial message is the skill content, prefixed with:
   ...
   - A statement that the spawned agent is already `trellis-check` and must review/fix directly without spawning another `trellis-check` / `trellis-implement`
